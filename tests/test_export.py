...
def test_export_creates_required_sheets_and_summary_formulas(tmp_path) -> None:
    ...
    assert getattr(summary["D6"].border.top, "style", None) is None
    assert summary["E6"].border.top.style == "medium"
    assert getattr(summary["H6"].border.top, "style", None) is None
    ...
    assert summary.row_dimensions[23].height == 25
...
def test_export_places_other_input_tax_and_deductions_before_sales(tmp_path) -> None:
    ...
    assert summary.row_dimensions[12].height == 25
    assert summary.row_dimensions[14].height == 25
    assert getattr(summary["A12"].border.bottom, "style", None) is None
    assert getattr(summary["A12"].border.top, "style", None) is None
